...
if __name__ == '__main__':
  ...
  if MODEL == 'e':
    ...
    faceRecognizer.load('face_model_eigen.yml')
  elif MODEL == 'f':
    # Create a face recognizer object
    faceRecognizer = cv2.face.createFisherFaceRecognizer()
    # load model from disk
    faceRecognizer.load('face_model_fisher.yml')
  elif MODEL == 'l':
    # Create a face recognizer object
    faceRecognizer = cv2.face.createLBPHFaceRecognizer()
    # load model from disk
    faceRecognizer.load('face_model_lbph.yml')
  ...
  while VIDEO or (i < len(testFiles)):
    ...
    cv2.putText(original, labelsMap[predictedLabel], (10,100), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2);
