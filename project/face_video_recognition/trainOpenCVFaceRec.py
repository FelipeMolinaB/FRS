...
def alignFace(imFace, landmarks):
    ...
    dy = r_y - l_y
    dx = r_x - l_x
    angle = math.atan2(dy, dx) * 100.0 / math.pi  # Convert from radians to degrees
...
if __name__ == '__main__':
  ...
  faceRecognizerEigen = cv2.face.createEigenFaceRecognizer()
  faceRecognizerEigen.train(imagesFaceTrain, np.array(labelsFaceTrain))
  faceRecognizerEigen.save('face_model_eigen.yml')

  faceRecognizerFisher = cv2.face.createFisherFaceRecognizer()
  faceRecognizerFisher.train(imagesFaceTrain, np.array(labelsFaceTrain))
  faceRecognizerFisher.save('face_model_fisher.yml')

  faceRecognizerLBPH = cv2.face.createLBPHFaceRecognizer()
  faceRecognizerLBPH.train(imagesFaceTrain, np.array(labelsFaceTrain))
  faceRecognizerLBPH.save('face_model_lbph.yml')
